...
def error(expr_val,i):
    ...
    if expr_val:
        print(f"{WARNING}Erreur syntaxique, la chaîne n'a pas été parcourue en entier.Erreur près de l'unité lexicale {ENDC}\'{FAIL}{l[i][1]}{ENDC}\'{WARNING} à la position i = {i}{ENDC}")
    elif i == len(l):
        print(f"{WARNING}Erreur syntaxique près de l'unité lexicale {ENDC}\'{FAIL}{l[-1][1]}{ENDC}\'{WARNING} à la position i = {i}{ENDC}")
    else:
        print(f"{WARNING}Erreur syntaxique près de l'unité lexicale {ENDC}\'{FAIL}{l[i][1]}{ENDC}\'{WARNING} à la position i = {i}{ENDC}")
